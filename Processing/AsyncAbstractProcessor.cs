...
{
    /// <summary>
    /// Abstract base class for all asynchronous tanki processors
    /// </summary>
    public abstract class AsyncAbstractProcessor
    {
        ...
        private readonly Dictionary<int, Func<Command, Task<object>>> _commandHandlers;
        ...
        /// </summary>
        protected AsyncTankiSocket SocketInstance { get; private set; }
        ...
        /// <summary>
        /// Creates a new instance of AsyncAbstractProcessor
        /// </summary>
        ...
        protected AsyncAbstractProcessor(
            AsyncTankiSocket socket,
            ...
            Dictionary<string, object> credentials)
        {
            ...
            _commandHandlers = new Dictionary<int, Func<Command, Task<object>>>();
        ...
        protected void RegisterHandler<TCommand>(Func<TCommand, Task<object>> handler) where TCommand : Command, new()
        {
        ...
        protected async Task<object> HandleCommandAsync(Command command)
        {
        ...
        /// </summary>
        protected abstract Task ProcessPacketsAsync();

        /// <summary>
        /// Handles successful login
        /// </summary>
        protected abstract Task OnLoginAsync();

        /// <summary>
        /// Parses and processes received packets
        /// </summary>
        /// <param name="packet">The packet to process</param>
        public async Task ParsePacketsAsync(AbstractPacket packet)
        {
            CurrentPacket = packet;

            if (!await ProcessUniversalPacketsAsync() && !await ProcessEntryPacketsAsync())
        ...
        private async Task<bool> ProcessUniversalPacketsAsync()
        {
            ...
            else if (ComparePacket("Load_Resources"))
            {
                ...
                await SendPacketAsync(loadedPacket);
            }
            else
            {
                return false;
        ...
        /// </summary>
        /// <returns>True if the packet was processed, false otherwise</returns>
        private async Task<bool> ProcessEntryPacketsAsync()
        {
            var packetObject = CurrentPacket.Object;
            ...
            else if (ComparePacket("Set_Captcha_Keys"))
            {
                ...
                await SendPacketAsync(clientLangPacket);
            }
            else if (ComparePacket("Invite_Code_Status"))
            {
                if ((bool)packetObject["inviteEnabled"])
                    await CloseSocketAsync("Invite code required");
            }
            ...
            {
                await CloseSocketAsync("Login Failed", addToReconnections: false, killInstance: true);
            }
            else if (ComparePacket("Banned"))
            {
                await CloseSocketAsync("Account Banned", addToReconnections: false, killInstance: true);
            }
            else
            {
                return false;
        ...
        protected async Task SendPacketAsync(AbstractPacket packet)
        {
            ByteArray buffer;
            lock (_sendLock)
            {
            ...
            await SocketInstance.SendAsync(buffer);
        }
        ...
        protected async Task CloseSocketAsync(string reason, bool logError = true, bool addToReconnections = true, bool killInstance = false)
        {
            if (logError)
            ...
            {
                await SocketInstance.CloseSocketAsync();
            }
        ...
        {
            CreateTimer(deltaTime, () => SendPacketAsync(packet).Wait());
        }
        ...
        public void SetSocketInstance(AsyncTankiSocket socket)
        {
            SocketInstance = socket;
        }
    }

...
public class VectorVector3DCodec : BaseCodec
{
    ...
    /// </summary>
    public static VectorVector3DCodec Instance { get; } = new(Vector3DCodec.Instance);
    ...
    public override object? Decode(EByteArray buffer)
    {
        ...
        for (int i = 0; i < length; i++) result.Add((Vector3D)_vector3DCodec.Decode(buffer));
    ...
    public override int Encode(object? value, EByteArray buffer)
    {
        ...
        foreach (var vector in list) bytesWritten += _vector3DCodec.Encode(vector, buffer);

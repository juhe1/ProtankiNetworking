...
public class VectorShortCodec : BaseCodec
{
    ...
    /// </summary>
    public static VectorShortCodec Instance { get; } = new(ShortCodec.Instance);
    ...
    public override object? Decode(EByteArray buffer)
    {
        ...
        for (int i = 0; i < length; i++) result.Add((short)_shortCodec.Decode(buffer));
    ...
    {
        if (value is not List<short> list) throw new ArgumentException("Value must be a list of shorts", nameof(value));

        var bytesWritten = 0;
        buffer.WriteInt(list.Count);
        bytesWritten += 4;

        foreach (var val in list) bytesWritten += _shortCodec.Encode(val, buffer);
